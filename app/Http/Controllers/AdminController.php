...
class AdminController extends Controller
{
    ...
    public function categoriesadmin()
    {
        ...
        return view('DoAN_nhomF.admin.categories', compact('cate'));
    }

    public function from_add_category()
    {
        return view('DoAN_nhomF.admin.form_add_category');
    }
    public function post_from_add_category(Request $request)
    {
        // // $input = $request->all();
        // $input = $request->all();
        // $cate= Category::paginate(2);
        $request->validate([
            'category_name' => ['required', 'unique:categories,category_name', 'string', 'max:255', 'regex:/^[\p{L}0-9 ]+$/u'],
            'description' => 'required|string|max:255',
        ], [
            'category_name.required' => 'Tên danh mục là bắt buộc.',
            'category_name.unique' => 'Tên danh mục đã tồn tại',
            'category_name.max' => 'Tối đa 255 kí tự .',
            'code.regex' => ' Tên danh mục gồm gồm chữ và số có dấu và có khoảng trắng ',
        ...
        try {
            ...
            return redirect('categoriesadmin')->with('success', 'Thêm Category thành công.');
        } catch (\Illuminate\Database\QueryException $e) {
            // Bắt lỗi duplicate code
            if ($e->errorInfo[1] == 1062) {
                return redirect()->back()
                    ->withErrors(['category_name' => 'Tên danh mụcnày đã tồn tại, vui lòng nhập Tên danh mục khác.'])
                    ->withInput();
            }

            // Lỗi khác
            return redirect()->back()
                ->withErrors(['msg' => 'Đã xảy ra lỗi khi thêm Category.'])
                ->withInput();
        }
    }

    public function from_update_category(Request $request)
    {
        $category_id = $request->get('category_id');
        $cate = Category::find($category_id);

        return view('DoAN_nhomF.admin.from_update_category', compact('cate'));
    }

    public function post_from_update_category(Request $request)
    {
        $input = $request->all();
        // Tìm cate theo ID
        $cate = Category::find($input['category_id']);

        // Kiểm tra xung đột cập nhật bằng updated_at
        if (optional($cate->updated_at)->toDateTimeString() !== $input['updated_at']) {
            $changedFields = [];
            if ($cate->category_name !== $input['category_name']) $changedFields[] = 'tên thể loại';
            if ($cate->description !== $input['description']) $changedFields[] = 'Mô tả';
        ...
        $request->validate([
            'category_name' => [
                ...
                'max:255',
                'regex:/^[\p{L}0-9 ]+$/u',
                Rule::unique('categories', 'category_name')->ignore($cate->id),
            ],
            'description' => 'required|string|max:255',
        ], [
            'category_name.required' => 'Tên danh mục là bắt buộc.',
            'category_name.unique' => 'Tên danh mục đã tồn tại.',
            'category_name.max' => 'Tối đa 255 kí tự.',
            'category_name.regex' => 'Tên danh mục chỉ được chứa chữ, số có dấu và khoảng trắng.',
        ...
        return redirect('categoriesadmin')->with('success', 'Cập nhật Category thành công.');
    }
